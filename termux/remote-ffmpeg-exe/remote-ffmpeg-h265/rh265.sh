...
getLogPath(){
    ...
    echo "${CUR_DIR}"/logs/${REMOTE_TMPFILE}-pid-$$.log
}
echoAndLog(){
    echo $@
    echo  "$@"  >>  "`getLogPath`"
...
endLog(){ # 程序结束时，归档日志
    mkdir -p "${CUR_DIR}"/logs/end
    mv "`getLogPath`"  "${CUR_DIR}"/logs/end
}
...
echo "tail -f -n 100 `getLogPath`" > $LOG_SHOTCUT
...
# 上传 
echoAndLog "上传中... ${VDNAME}  =>  远程目录${REMOTEDIR}/${REMOTE_TMPFILE}.input"
uploadTo(){
    sshpass -p "${PASSWD}" ssh -l $USER -p $PORT $HOST "mkdir -p ${REMOTEDIR}" # 创建远程目录
    sshpass -p "${PASSWD}" rsync -avP -e "ssh -p ${PORT}" "${VDNAME}" ${USER}@${HOST}:"${REMOTEDIR}/${REMOTE_TMPFILE}.input" 2>&1 |tee -a `getLogPath` # 上传  （注意，多个了P参数，支持断点续传）
...
while true; do
    if [[ -e "${VDNAME}${GENF_SUFFIX}.input.md5" ]]; then
        ...
        if [[ $(cat "${VDNAME}${GENF_SUFFIX}.input.md5") = '' ]]; then
            echoAndLog "他妈的，这是一种极端情况，如果没有这一条拦截，就被混过去了。（以前无缘无故在远程没有input文件的情况下，直接跳到了第二步的ffmpeg命令去了！）"
            uploadTo
            continue
        fi
        
        if [[ $(cat "${VDNAME}${GENF_SUFFIX}.input.md5") = $(md5sum "${VDNAME}"|awk '{print $1}') ]]; then 
            echoAndLog "已确认完整上传: ${VDNAME}"
            break  # 确保完整上传后，才可跳出重试的循环
        else
            echoAndLog "上传失败，现在重试..."
            uploadTo
        fi
    else
        echoAndLog C
...
echo "sshpass -p '${PASSWD}' ssh -l $USER -p $PORT $HOST 'tail -f -n 100 ${REMOTEDIR}/${REMOTE_TMPFILE}.nohup'; " > ${VDNAME}${GENF_SUFFIX}.remotelog.sh # 提供一条看远程日志的命令
...
while true; do
    ...
    if [[ -e "${VDNAME}${GENF_SUFFIX}.output.size" ]]; then
        echoAndLog '进行中, 远程结果文件大小：'`cat "${VDNAME}${GENF_SUFFIX}.output.size"`
    fi
    # 检查是否完成
...
echoAndLog "转码完毕，开始下载结果...  ${USER}@${HOST}:${REMOTEDIR}/${REMOTE_TMPFILE}.output.md5  => ${dlPath}.mkv "
downloadResult(){
    sshpass -p "${PASSWD}" rsync -avP -e "ssh -p ${PORT}" ${USER}@${HOST}:"${REMOTEDIR}/${REMOTE_TMPFILE}.output"  "${dlPath}"  2>&1 |tee -a `getLogPath`  # 注意，多个了P参数，支持断点续传
...
while true; do
    if [[ -e "${dlPath}" ]]; then
        if [[ $(cat "${VDNAME}${GENF_SUFFIX}.output.md5") = $(md5sum "${dlPath}"|awk '{print $1}') ]]; then 
            ...
            rm "${VDNAME}${GENF_SUFFIX}.input.md5" "${VDNAME}${GENF_SUFFIX}.output.md5" "${VDNAME}${GENF_SUFFIX}.output.size" "${VDNAME}${GENF_SUFFIX}.finished" "${VDNAME}${GENF_SUFFIX}.remotelog.sh" -f
            echoAndLog "取回完毕： ${dlPath}"
            break
        else
            downloadResult
        fi
    ...
    sleep 30
    echoAndLog '临时改的代码，下载的重试循环也等30秒，防止在户外网络不稳定时频繁下载浪费流量'
done
