...
if [ $count -eq 1 ]
then 
    ffmpeg.exe -i $safedir/1.flv -c copy $safedir/result/merge.mp4
    rm $safedir/1.flv -f
else
    ...
    while(( $i<=count )) 
    do
        ffmpeg.exe -i $safedir/$i.flv -c copy $safedir/$i.mp4
        rm $safedir/$i.flv -f
    ...
    # 开始合并mp4
    ffmpeg.exe -f concat -i $listfile -c copy $safedir/result/merge.mp4
